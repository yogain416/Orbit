    ...
    <div className="flex items-center justify-center h-screen bg-gradient-to-br from-indigo-50 via-white to-slate-50">
      <div className="w-full max-w-md mx-auto px-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
          ...
          {waitingCallback && (
            <div className="mt-5 px-3 py-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800 leading-relaxed">
              브라우저에서 Google 로그인을 완료해 주세요. 인증이 끝나면 자동으로 이 창으로 돌아옵니다.
            </div>
          )}

          {error && (
            <div className="mt-5 px-3 py-3 bg-rose-50 border border-rose-200 rounded-lg text-xs text-rose-800 leading-relaxed">
              {error}
            </div>
          )}
